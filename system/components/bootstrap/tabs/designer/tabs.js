...
define(function(require) {
	...
	var quickIdeEx = {
			"$UI/system/components/bootstrap/tabs/tabs(bootstrap)":{
		        ...
		        operations:function(){
		        	return [{groupName:"页签(流式)",
		        		items:[{name:'setActiveTab',label:"跳转到",argExtends:{tab:{editor:'jsExpressionEX'}}}
		        		]}];
		        }
			},
			"$UI/system/components/bootstrap/tabs/tabs(bootstrap)#tabItem":{
			...
			"$UI/system/components/bootstrap/tabs/tabs(bootstrap)#tabNav":{
				styleConfig:function(designer){
					return [
					        designer.getCommonStyleItem('baseStyle'),
							designer.getCommonStyleItem('font'),
	...
	return {
		...
		'$UI/system/components/bootstrap/tabs/tabs(bootstrap)#tabItem' : TabItem,quickIdeEx:quickIdeEx
	};
