...
public class RelationDatasource extends PropertyDatasource {
	...
	public DataSet getDatasource(PropertyItem propertyItem) {
		...
		/*如果是data节点或data的子节点*/
		if(("$UI/system/components/justep/data/bizData".equals(xuiElement.getName())) && ("data".equals(xuiElement.getConfigAttribute("component-type")) || "data".equals(xuiElement.getParentElement().getConfigAttribute("component-type")))){
		...
		else if("$UI/system/components/justep/data/data".equals(xuiElement.getName()) || "$UI/system/components/justep/data/baasData".equals(xuiElement.getName())){
				DataSet ds = XuiDataSourceManager.createHasRelationDataSet();
				List<XuiElement> childList = xuiElement.getChildListByTagName("column");
