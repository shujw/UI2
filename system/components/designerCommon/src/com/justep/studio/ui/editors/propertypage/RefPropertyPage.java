...
//	private String dataItemAlias = null;
	private String defaultPage = "ref";
...
//			else
//				if(this.dataItemAlias != null && !this.dataItemAlias.equals("")){//有别名的情况.
//					if(str.startsWith(this.dataItemAlias+".")){
//						return str;
//					}
//				}
//				return "$model."+str;
	...
	private void findForeachContext(XuiElement xuiElement){
		...
		if(null!=xuiE){
			...
			if(element.hasAttribute("bind-foreach")){
				...
				if(foreachData.equals("")){
					MessageDialog.openWarning(this.getShell(), "警告", "当前组件所在的list没有关联data，请先设置后再进行操作！");
				}
			}else findForeachContext(xuiE);
		
			String listCompName = "$UI/system/components/justep/list/list";
			if(foreachData!=null && !foreachData.equals("") && listCompName.equals(PRange)){
				 XuiElement parent = xuiE.getParentElement();
				 while(parent != null){
					// System.out.println("===========>"+parent.getProperyValue("component"));
					 if(listCompName.equals(parent.getProperyValue("component"))){
						 break;
					 }
					 parent = parent.getParentElement();
				 }
//				 if(parent != null){
//					this.dataItemAlias =  parent.getProperyValue("dataItemAlias");
//				 }
			}
		}
	}
	...
	private void createContents(){	
		...
		if(hasLabel){
			...
			if("label".equalsIgnoreCase(this.defaultPage)){
				tabFolder.setSelection(tabItem2);
				this.selectedRef = false;
			}
		}
	...
	public void setPropertyItem(PropertyItem propertyItem) {
		...
		if(null!=this.editorParameter){
			...
			String defaultPage = this.editorParameter.get("default");
			if(null!=defaultPage && "label".equalsIgnoreCase(defaultPage) && this.hasLabel){
				this.defaultPage = "label";
			}
		...
		this.dataset = DataExpressUtil.buildVarDataset(xuiElement, propertyItem.getName(),this.isRelation,this.data);//(DataSet)XuiDynJavaManager.executeMethod(propertyItem.getOwnerElementBasePath(), className, methodName,new Object[]{propertyItem});
//		
//		if(null!=data && !"".equals(data)){
...
//		if(this.dataItemAlias != null && !this.dataItemAlias.equals("")){
//			for(DataRecord rc:list){
//				String xid = rc.getString("xid");
//				if(this.foreachData.equals(xid)){
//					
//					DataRecord dataNode = this.dataset.addRecord(new Object[]{ "dataAlias", dataItemAlias,dataItemAlias });;
//					dataNode.setValue("image", "data.gif");
//					List<DataRecord> childList = rc.getChildList();
//					for(DataRecord childRc:childList){
//						String text = childRc.getString("text");
//						String _xid = childRc.getString("xid");
//						DataRecord relNode = new DataRecord(new Object[] { "relation", text, _xid }, dataNode);
			...
			this.dataset.addDataSetChangedListener(new DataSetChangedListener(){
				public void dataSetChanged(DataSetChangedEvent event) {
					if(event.getEventType() == DataSetChangedEvent.DATACHANGED || event.getEventType() == DataSetChangedEvent.SELECTIONCHANGED){
						...
						if(selectedRef){
							String str = DataExpressUtil.getRefPath(rcList.get(0));
							if(isRelation){
								int idx = str.indexOf("\"");
								if(idx >0){
									str = str.substring(idx+1,str.indexOf("\"",idx+1));
								}
							}
							displayText.setText(str);
						}else{
							String str = DataExpressUtil.getLabelPath(rcList.get(0));
							displayText.setText(str);
						}
