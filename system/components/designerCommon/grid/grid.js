		...
		init : function(value, bindingContext) {
			...
			if (sCfg) {
				...
				if (cfg) {
					...
					// 创建grid组件
					$e.jqxGrid(cfg);
					var widget = $.data(this.domNode)["jqxGrid"].instance, crow = data.getCurrentRow(true);
					// 设置初始的选中行
					...
					$(this.domNode).on('rowselect', function(event) {
						if (!widget['data-updating']) {
							...
							try {
								var evt = event.args;
								var crow = data.getCurrentRow(true), irow = data.datas.get()[evt.rowindex];
								if (crow !== irow) {
									data.to(irow);
									crow = data.getCurrentRow(true);
