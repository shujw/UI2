define(function(require) {
	...
	mng.callJava = function(data, filePath) {
		if(!this.getRequestParameter("port")){
			return;
		} 
		...
		if (data.async !== false) {
			...
			$.ajax({
				...
				success : function(result) {
					if(result.$data !== ""){
						returnData = (result.$data === undefined?result:result.$data);
					}
				},
	...
	if(navigator.userAgent.toLowerCase().match(/chrome/) != null &&  mng.getRequestParameter("port") && !mng.webIde){
		//重写alert
		var oldAerlt = window["alert"];
	    window["alert"] = function(msg){
	    	mng.callJava({alertMsg:msg,async:false});
	    }
	    
	    //重写confirm
		var oldConfirm = window["confirm"];
	    window["confirm"] = function(msg,title){
	    	var result = mng.callJava({confirmMsg:msg,async:false});
	    	return result===true;
	    }
	}
	return mng;
