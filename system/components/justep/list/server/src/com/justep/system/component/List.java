...
public class List implements ComponentTemplate{
	...
	protected Element getTemplate(){
		String css = eDef.attributeValue("class");
		if(null!=css && !"".equals(css) && css.indexOf("x-list-template")>0) return eDef;
	...
	protected void processElement(){
		...
		if(null!=eT){
			...
			//if(null!=data && !"".equals(data)){
				String dataItemAlias = eDef.attributeValue("dataItemAlias");
				if(null!=dataItemAlias && !"".equals(dataItemAlias)){
					dataItemAlias = ",as:'"+dataItemAlias+"'";
				}else dataItemAlias = "";
				eT.addAttribute("bind-foreach", "{data:"+getForeachFuncName()+",afterRender:"+getForeachAfterRenderFuncName()+dataItemAlias+"}");				
			//}
		}
	}
