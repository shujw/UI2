...
define(function(require) {
	...
	//dataTable:1.10.10  responsive:2.0.0
	require("css!./css/dataTables.bootstrap").load();
	require("css!./css/responsive.dataTables").load();
	require("css!./css/responsive.bootstrap").load();
	...
	require("./js/responsive.bootstrap");
	...
	var parseHTML= function(html){
		if(typeof(html)==='string'){
			return html.replace(/ on/,' on ').replace(/\&/gm,"&amp;").replace(/\>/gm,"&gt;").replace(/\</gm,"&lt;");//存在一定隐患
		}else return html;
	};
	...
	var valToString = function(v, colDef){
		...
		else if(colDef.format && ('Integer' == colDef.dataType 
				|| 'Long' == colDef.dataType
				|| 'Float' == colDef.dataType
				|| 'Double' == colDef.dataType
				|| 'Decimal' == colDef.dataType)){
			return (new DecimalFormat(colDef.format)).format(v);
		}
		else return v;
	};
	...
	var DataTables = justep.ViewComponent.extend({
		...
		dispose : function() {
			...
			if (this.$domNode && this.$domNode.dataTable && this.$domNode.dataTable().fnDestroy)
				this.$domNode.dataTable().fnDestroy();
			this.callParent();
		},
			...
			var cfg = $.extend({
				...
				"fnServerData" : function(sSource, aoData, fnCallback, settings){
					if(data){
						...
						var retDatas = self.doFilter(data.datas.get().slice(0));
						fnCallback({data:retDatas,recordsTotal:recodeCount,recordsFiltered:recodeCount});
		...
		_clearResponsiveResizeHandle: function(){
			if (this._responsiveResizeHandle){
				window.clearTimeout(this._responsiveResizeHandle);
				this._responsiveResizeHandle = null;
			}
		},
		_responsiveResize: function(){
			this._clearResponsiveResizeHandle();
			var self = this;
			this._responsiveResizeHandle = window.setTimeout(function(){
				self._clearResponsiveResizeHandle();
				self.dataTableApi().responsive.recalc();
			}, 100);
		},
			...
			if(this.useFooter){
				...
				$.each(colDefs,function(i,v){
					if(v.footerData){
						...
						//使用定义的格式化valToString
						footerData[v.sName] = valToString(justep.Express.eval(v._footerDataExpr, ctx.$object, ctx),v);
						delete ctx.$data;
					}
				});
		...
		_processColCfg : function(cfg){
			...
			cfg["columnDefs"].push({
				...
				render: function(val, type, row, meta) {
					...
					//增加对data的extendType的支持
					var data = row?row.data:null;
					if(data){
						//var html = Data.getDisplayHtml(data.getModel(),row.ref(evtData.colName),{comp:self,type:'dataTables'});
						var r=row,html,model = self.getModel(),ref = r.ref(evtData.colName);
						if(Data.getExtendTypeObject){
							var extendTypeObj = Data.getExtendTypeObject(model, ref);
							if(extendTypeObj){ 
								var datacolDef = Data.getColDef(ref);
								...
								if(Editor && (Editor.getDisplayHtml||Editor.getDisplayText)){
									context = new justep.ViewComponent.DisplayContext({comp:self,type:'dataTables',colDef:datacolDef,row:r,extendType:extendTypeObj});
		...
		_bindEvent : function(on){
			...
			if(this.multiSelect){
				...
				this.$domNode.find('tbody')[fn]('click', 'td.dataTables-cb input', function(evt){
					...
					if(self.hasListener('onRowChecked')){
						//多选勾选事件
						evtData = {source:self,row:api.row(tr).data(),domEvent: evt,checked:input.checked};
						evtData.rowID = evtData.row.getID();
				...
				this._eventHandles['__data_change__'] = function(event){
					...
					if(!self._disON && event.selfChanged
						...
						if(event.type === 'refresh' || event.type === 'exchangeRow'){
							...
							self.removeRow(event.row);
							if(data.getCount()<=0) self._lastsel = null;
			...
			model.on("onPullUp", function(evt) {
				if (!this.disableInfiniteLoad && this._isSelfRefresh(evt.source)) {
					this.hasMore = this.loadNextPageData();
		...
		loadNextPageData : function() {
			var data = this.getData();
			if (-1 !== data.limit && this.hasMore){
				data.loadNextPageData({
					append : true
				});
				return data.getTotal() > data.getOffset();
			}return false;
		},
		...
		_isSelfRefresh : function(refreshComp) {
			return this.$domNode.parents('.x-scroll')[0] == refreshComp.domNode;
		},
		...
		getDataRow: function(node){
			var $tr = $(node, this.domNode.rows).closest("tr");
			if($tr.size()>0){
				var $table = this.dataTableApi();
