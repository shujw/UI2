...
public class Window extends BaseComponent {
	String resourceCompName = "$/UI2/system/components/justep/resource";
	 ...
	 * @param currentE 当前节点
	 * @param filtStr  文件后缀
	 * @param prefix  结果上加上的前缀
	 */
	@SuppressWarnings("unchecked")
	public void addRequire(XuiElement currentE,String filtStr,String prefix){
		XuiDataModel model = currentE.getXuiDataModel();
		currentE = model.getRootElement().getChildByName(resourceCompName);
		if(currentE == null){
			 String dId = model.genaId(resourceCompName);
			 model.getDesigner().createComponent(resourceCompName, model.getRootElement().getDesignId(), dId, null, null, true);		
			 currentE =  model.findElementByDesignId(dId);
		}
		...
		model.getDesigner().repaintComponent(newE);
