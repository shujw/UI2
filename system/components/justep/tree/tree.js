	...
	var Tree = List
			.extend({
				...
				dispose : function() {
					...
					if(data) data.off('onAfterRefresh', this._doDataRefresh);
					if(bar) bar.off('onClick', this._doBarClick);
					this.callParent();
				},
				...
				_doRowLinkClick : function(evt) {
					...
					if (!eData.cancel && !data.isLeaf(row)) {// 叶子节点不展开
						var async = false;
						var fn = function(data,row,forcedTo){
							if (row && ((row.rows && row.rows.get().length > 0)||forcedTo)) {// 有数据进入 
								if(row.rows && row.rows.get().length > 0) data.to(row.rows.get()[0]);
						...
						}.bind(this,data,row,eData.forcedTo);
				...
				getForeachData : function() {
					...
					if (context.datas) {
						...
						if (data instanceof Data) {
							...
							var size = datas.length, offset = (-1 == this.limit ? size : (list_limit * context.pageIndex)), total = data
