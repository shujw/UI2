...
define(function(require) {
	...
	CellLayout.prototype = {
		...
		showMenu:function(event){
			if(!this.ownerDesigner.showMenu){
				return;
			}
			var target = event.target;
			var offset = $(this.ownerDesigner.element).offset();
			var top = event.clientY - offset.top;
			var left = event.clientX - offset.left;
			this.ownerDesigner.showMenu(this.domNode,left,top);
		},
...
//			});
			...
			$(this.domNode).on("childChanged",function(event){ 
				setTimeout(function(){ 
			...
			for ( var i = 0, l = childNodes.length; i < l; i++) {
				...
				if (node.tagName.toLowerCase() == 'layout-content') {
					if(this.ownerDesigner.dataModel){
						layoutData = this.ownerDesigner.dataModel.getPropValue(this.domNode.getAttribute("d_id"),"layout-data");
						if(layoutData){
		...
		copyCells : function(config) {
			if (this.selections.length > 1) {
				...
				for ( var i = 0, l = cells.length; i < l; i++) {
					...
					row.push(this.genaCellHtml(cell, null, null, null,true));
				}
		...
		pasteCells : function(config) {
			...
			if (cellXml) {
				...
				if(cellXml.substring(0,6) != "<table"){ 
//					clientParent modelParentDid parentDid
					var node = this.ownerDesigner.buildNewNodeXml(cellXml); 
					var componentName = node.getAttribute("componentName"); 
					content = xmlUtil.toString(node); 
					content = content.replace('xmlns="http://www.w3.org/1999/xhtml"',''); 
					var modelNode = this.ownerDesigner.dataModel.createElement({clientParent:firstCell,modelParentDid:this.domNode.getAttribute("d_id"),componentName:componentName,templateContent:content});
					
					firstCell.setAttribute("componentId",modelNode.getAttribute("xid"));		
					this.updateLayoutContent();
					return;
				} 
				cellXml = cellXml.replace(new RegExp("#C1C1C1|#c1c1c1", "gm"), "#010101");
				var xmlNode = xmlUtil.fromString(cellXml).documentElement;
				

				var cellData = $(cellXml)[0];
				$(cellData).find("*").each(function(){
					$(this).removeAttr("d_id");
				})
				...
				for ( i = 0; i < childNodes.length; i++) {
					...
					for ( var j = 0; j < cells.length; j++) {
						...
						if (cell.nodeType == 1) {
							...
							var compNodes = cell.childNodes;
							var xidBuf = [];
						 
							for(var n = 0,l=compNodes.length;n<l;n+=1){
								var compNode = compNodes[n];
								if(compNode.nodeType == 1){
									var xid = compNode.getAttribute("xid");		
									xidBuf.push(xid);
								}
							}
							
							for(var n = compNodes.length-1;n>=0;n-=1){
								var compNode = compNodes[n];
								if(compNode.nodeType == 1){
									$(compNode).remove();									
								}
							...
							if(xidBuf.length>0){
								cell.setAttribute("componentId",xidBuf.join(","))								
							}
							
							var td = $(cell.outerHTML).insertBefore(rows[i][0])[0];
							td.setAttribute("id", "_id")
							for(var n = 0;n<xidBuf.length;n+=1){
								var xid = xidBuf[n];		
								var modelNode = $(xmlNode).find("*[xid='"+xid+"']:first")[0];
								if(modelNode){
									var xml = xmlUtil.toString(modelNode);
									compConfig.push({clientParent:td,parent:this.domNode,templateContent:xml,componentName:modelNode.getAttribute("componentName")});
								}
							}
						}
					}
				...
				var self = this;
				// var html = this.asHtml();
				for(var i = 0;i<compConfig.length;i+=1){
					this.ownerDesigner.createComponent(compConfig[i]);
				}
				this.currentCell = this.selections[0].getCells()[0];
				this.updateLayoutContent(comIds);
		...
		updateLayoutContent : function(removeComIds, paths) { 
			...
			if(this.ownerDesigner.dataModel){
				if(removeComIds){
					this.ownerDesigner.dataModel.stopUndoRedo = true;
					this.ownerDesigner.removeSelections({ids:removeComIds});
					this.ownerDesigner.dataModel.stopUndoRedo = false;
				}			 
				this.ownerDesigner.dataModel.setPropValue(this.domNode.getAttribute("d_id"),"layout-data",html,true,"updateLayoutContent");	
				//self.updateAllSelectionUnitBound();
			}
...
//			});
		...
		/** 生成单元格的html源码* */
		genaCellHtml : function(cell, rowIndex, colIndex, keyComponentIdAttr,genaCompXml) {
			...
			return "<td " + attr + ">" + txt +compXml.join("")+ "</td>";
		},
		...
		css:function(dom,cssConfig){
			...
			for(var i = 0;i<styleItems.length;i+=1){
				var item = styleItems[i];
				if(item){
					var items = $.trim(item).split(":");
					if(items.length == 2){
						oldCssConfig[items[0]] = items[1];
					}
				}
			}
			
			for(var p in cssConfig){
				var v = cssConfig[p];
				if(v == 'remove'){ 
					delete oldCssConfig[p];
					delete cssConfig[p];
				}else{
					oldCssConfig[p] = cssConfig[p];					
				}
			}
			
			var cssBuf = [];
			for(var p in oldCssConfig){
				var v = oldCssConfig[p];
				if(v){
					cssBuf.push(p+":"+oldCssConfig[p]);					
				}
			}
			//if(cssBuf.length>0){
				var style = cssBuf.join(";");
		...
		getCurrentCellStyle : function(){
			if (!this.selections) {
				return "";
			} 
			var cells = this.selections[0].getCells();
			return cells[0].getAttribute("style");
		},
...
//									config["border-width"] = cell.style.borderWidth || "1px";
//								}
								
//								if (cell.style.borderColor === "") {
//									config["border-color"] = cell.style.borderColor||"#000000";
//								}
								var style = config["border-style"] || cell.style.borderStyle || "solid";
								var width = config["border-width"] || cell.style.borderWidth || "1";
								var color = config["border-color"] || cell.style.borderColor || "#000000";
								var newConfig = {"cell-layout-border-style":{style:"border_all",pattern:style,width:width,color:color}}; 
								
								this.setCellStyle(newConfig);
//							}
				...
				var style = config["cell-layout-border-style"];
//				eval("style=" + config["cell-layout-border-style"]);
			...
			if (!this.getSelectionUnit(cell)) {
				...
				this.selctionChangeCaller = setTimeout(function() {
					if (self.ownerDesigner) {
						...
						self.ownerDesigner.updatePropEditor(dId,"cellLayoutCell",{style:cell.getAttribute("style")},function(){
		...
		deleteCells : function(cells/* 可以是一个数组或者单个单元格对象 */, parent, onlyClearContent, comIds, paths) {
			...
			try {
				...
				if (!comIds) {
					...
					if (comIds.length > 0) {
						this.ownerDesigner.removeSelections({ids : comIds});
					}
				}
		...
		clearCellContent : function() {
			...
			for ( var i = 0; i < l; i++) {
				var cells = this.selections[i].getCells();
				this.deleteCells(cells, null, true);
			}	
			if (l > 0) {
				this.updateLayoutContent();
			}
		...
		mergeCell : function() {
			...
			for (i = 0, l = this.selections.length; i < l; i++) {
				...
				this.deleteCells(selectedCells, null, null);
		...
		deleteRow : function() {
			...
			for (i = rows.length - 1; i >= 0; i--) {
				...
				this.deleteCells(row.cells, row, null);
		...
		deleteCol : function() {
			...
			for (i = options.length - 1; i >= 0; i--) {
				...
				if (options[i].del) {
					this.deleteCells(row.cells[cellIndex], row, null);
				} else {
	...
	var quickIdeEx = {
			"$UI/system/components/justep/cellLayout/cellLayout":{
				styleConfig:function(designer){
					return [ designer.getCommonStyleItem('font'),
	...
	return {
		'$UI/system/components/justep/cellLayout/cellLayout' : CellLayout,quickIdeEx:quickIdeEx
		};
});