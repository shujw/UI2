...
*/	
    ...
    var Panel = ViewComponent.extend({
        ...
        init: function(value, bindingContext){
        	...
        	this.setBackgroundImage(this.buildSrc(this.$content.attr('backgroundImage')));
		...
		setBackgroundImage : function(url){
			this.$content.css('background-image',url ? "url(" + url + ")" : "");
			this.$content.css('backgroundSize',"cover");
		},
        /**
         * 设置上下区域的高度
         * @method setHeight 
        ...
        remove: function(name){
        	var $node = $('.x-panel-' + name, this.$el);
        	if($node.size()>0) Component.removeNode($node[0]);
        }
    });
