...
//								+ data.getValue('required', param.row) + ']]></expr><message '+(self.xids['col_'+colName+'_required_message_XID']?('xid="'+self.xids['col_'+colName+'_required_message_XID']+'"'):'')+'><![CDATA['
//								+ (data.getValue('requiredMessage', param.row) ? data.getValue('requiredMessage', param.row) : '') + ']]></message></required>');
//						hasDef = true;
//					}
...
//	};
	...
	Model.prototype.getReturnValue = function() {
		var v = this.comp('dataReadonly').val();
		this.xmlModel.setValue("readonly>expr",v);
		return {
	...
	Model.prototype.init = function() {
		...
		"constraintMessage":"constraint>message"};//data列与节点之间的映射
		...
		this.comp('dataReadonly').val(this.xmlModel.getValue("readonly>expr"));
...
//	};
     ...
     */
		Model.prototype.openExpressionEditor = function(currentProp){
			...
			xuiService.openEditor("jSExpressionEditor", {d_id:this.d_id,value : comp.getValue(currentProp),propValue : comp.getValue(currentProp),propName:"dataColRule"}, function(result) {
		...
		Model.prototype.defaultValueExprClick = function(event){
			this.openExpressionEditor("defaultValue");
		};
		...
		Model.prototype.dataReadonlyExprClick = function(event){
			...
			xuiService.openEditor("jSExpressionEditor", {value : comp.val(),propValue : comp.val(),propName:"dataRule"}, function(result) {
