...
define(function(require) {
	...
	var RTRestData = require("../restData");
	...
	var RestData = RTRestData.extend({
		...
		newFilter : function(config,designer) { 
			xuiDoc.createComponent("$UI/system/components/justep/data/restData#filter", this, {
				autoSelect:true,
				defaultProperties : {
	...
	RestData.designerConfig = {
			
	}
	var quickIdeEx = {
			"$UI/system/components/justep/data/restData":{
				properties:["label","orderBy",{name:"className",readonly:true},{name:"onCustomNew",text:"新增事件",editor:"eventEditor",noLabel:true},{name:"onAfterNew",text:"新增后事件",editor:"eventEditor",noLabel:true},{name:"onCustomDelete",text:"删除事件",editor:"eventEditor",noLabel:true},{name:"onAfterDelete",text:"删除后事件",editor:"eventEditor",noLabel:true},{name:"onCustomRefresh",text:"刷新事件",editor:"eventEditor",noLabel:true},{name:"onAfterRefresh",text:"刷新后事件",editor:"eventEditor",noLabel:true},{name:"onCustomSave",text:"保存事件",editor:"eventEditor",noLabel:true},{name:"onSaveCommit",text:"保存后事件",editor:"eventEditor",noLabel:true}],
				...
				getSubCompConfig:function(element){ 
					...
					if(element.parentNode && 
							element.parentNode.getAttribute("component") == "$UI/system/components/justep/data/restData"){ 
						 return this.subComps[componentName];
					} 
				},
				
				subComps:{
					"$UI/system/components/justep/data/restData#filter":{
						text:"过滤",
						displayName:function(values,name,idx){ 
							return "过滤"+(idx);
						},
						appendTo:">*[component='$UI/system/components/justep/data/restData']:first",//要添加到哪个节点下
	...
	return {
		'$UI/system/components/justep/data/restData' : RestData,quickIdeEx:quickIdeEx
	};
});