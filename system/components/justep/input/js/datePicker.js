...
define(function(require) {
	...
	function DatePicker(elm, dw, settings) {
		...
		this.show = function() {
			...
			//默认显示前后20年
			s.startYear = yy - 20;
			if(s.startDate && s.startDate.getFullYear()>s.startYear) s.startYear = s.startDate.getFullYear();
			s.endYear = yy + 20;
			if(s.endDate && s.endDate.getFullYear()<s.endYear) s.endYear = s.endDate.getFullYear();
			...
			// Set position
			$(window).on('resize.dw', function() {
				that.pos();
			});
			window.setTimeout(function(){that.pos();}, 10);//延迟显示重算，有键盘收起问题
		};
		...
		init : function(options) {
			...
			if ($('.dwo').length) {
				...
				var calc = function(t, val, isUp) {
					...
					//增加没有加载的年
					if(inst.settings.preset != 'time' && 0===i){
						var j;
						var s = inst.settings;
						var yy = parseInt(inst.temp[i], 10);
						var w = s.wheels[0];
						if(isUp && (s.endYear-yy)<=5){
							var endYear = s.endYear;
							s.endYear = yy + 15;
							if(s.endDate && s.endDate.getFullYear()<s.endYear) s.endYear = s.endDate.getFullYear();
							for (j = endYear+1; j <= s.endYear; j++){
								w[s.yearText][j] = s.dateOrder.search(/yy/i) < 0 ? j.toString().substr(2, 2) : j.toString();
								$('<li class="val_' + j + '">' + w[s.yearText][j] + '</li>').data('val', j).appendTo($(t));
							}
						}else if(!isUp && (yy-s.startYear)<=5){
							var startYear = s.startYear;
							s.startYear = yy - 15;
							if(s.startDate && s.startDate.getFullYear()>s.startYear) s.startYear = s.startDate.getFullYear();
							for (j = startYear+1; j >= s.startYear; j--){
								w[s.yearText][j] = s.dateOrder.search(/yy/i) < 0 ? j.toString().substr(2, 2) : j.toString();
								$('<li class="val_' + j + '">' + w[s.yearText][j] + '</li>').data('val', j).prependTo($(t));
								val -= h;
							}
						}
					}
