...
*/
	...
	var Wing = ViewComponent.extend({
		...
		buildTemplate: function(config){
			...
			if(config.routable) this.routable = config.routable;
			if(config.animate) this.animate = config.animate;
			if(config.display) this.display = config.display;
			if(config.dismissible) this.dismissible = config.dismissible;
			if(config.swipeClose) this.swipeClose = config.swipeClose;
			
			var style = config.style || '';

			return Str.format(
				"<div xid='{0}' class='{1}' component='{2}' style='{3}'>" +
					"<div class='x-wing-left' width='{4}'></div>" +
					"<div class='x-wing-content'></div>" +
					"<div class='x-wing-right' width='{5}'></div>" +
				"</div>", 
				config.xid, 
				this.baseCls,
				url,
		...
		init: function(value, bindingContext){
			...
			this.$backdrop = this.display !== 'compress'? 
		...
		addRouteItem: function(type,name){
			if(!this.routable){
				return;
			}
			...
			if(type === "show"){
				if(name == "content"){
					$routeState.removeState(xid,'left','');
					$routeState.removeState(xid,'right','');
				}else{
					$routeState.addState(xid,name,'');
				}
			}else if(type === "hide"){
				$routeState.removeState(xid,name);
			}
		},
		publishRouteItem:function(){
			if(!this.routable){
				return;
			}
			var $routeState = this.getModel().$routeState;
		...
		supportTouchSwip: function(){
			   // Touch events
			var swipeActiveArea = this.$content.width();
			var isTouched = false,isMoved = false,isFirstMove = false,touchesStart = {},isScrolling,
					touchesDiff,touchStartTime,deltaX,deltaY, self = this,
					touchMoveDirection,isFromBorder = false,isAndroidBrowser = $('html').hasClass('androidBrowser');
			this.$content.on("touchstart",function (e) {
				...
				if(/Android /.test(window.navigator.appVersion) && !(/Chrome\/\d/.test(window.navigator.appVersion))){
					...
					self.$right.hide();
					self.$content.hide();
					self.$left.css('display','');
					self.$content.css('display','');
					self.$right.css('display','');
				}
			});
			...
			this.$content.on("touchmove",function (e) {
				...
				var $scrollH = self.$content.find('.x-scroll-h');
				$scrollH.each(function(index,ele){
					if($(ele).find(e.target).length > 0){
				...
				if(isAndroidBrowser && timeDiff < 200){
					//优化低版本android效果
				}else{
			...
			this.$content.on("touchcancel touchend",function (e) {
				...
				if (deltaX === 0) {
					...
					self.addRouteItem('show','content');
					self.publishRouteItem();
					return;
				}
				var timeDiff = (new Date()).getTime() - touchStartTime;
				if ((timeDiff < 300 && deltaX > 30 ||timeDiff >= 300 && deltaX > DEFAULT_SIZE / 2) && isFromBorder) {
					...
					case 'overlay':
						if(touchMoveDirection == "right" && self.$left.length > 0){
							...
							self.addRouteItem('show','left');
							self.publishRouteItem();
						}else if(touchMoveDirection == "left" && self.$right.length > 0){
							self.rightOpened = true;
							self.addRouteItem('show','right');
							self.publishRouteItem();
						}
						break;
					case 'reveal':
					...
					self.$content.transitionEnd(function(){
						self.$content.transition(0);
						self.$left.transition(0);
						self.$right.transition(0);
					});
					
					var speed = deltaX/timeDiff;
					var leftTime = (DEFAULT_SIZE - deltaX)/speed;
					
					//console.log(leftTime);
					if(deltaX < 30 || (leftTime < 100 && deltaX > 30)){
						leftTime = 200;
					}
					self.$content.transition(leftTime);
					self.$left.transition(leftTime);
					self.$right.transition(leftTime);
					self.$content.transform('');
					self.$left.transform('');
		...
		supportTouchSwipHide: function(){
			   // Touch events
			var swipeActiveArea = this.$el.width();
			var isTouched = false,isMoved = false,isFirstMove = false,touchesStart = {},isScrolling,
					touchesDiff,touchStartTime,deltaX,deltaY,self = this,
					touchMoveDirection;
		...
		setSize: function(name, value){
			$('>.x-wing-' + name, this.$domNode).css('width', value);
		},
		/**
		 * 获取上下区域的高度
		 * @method getHeight
		...
		getSize: function(name){
			return $('>.x-wing-' + name, this.$domNode).css('width');
		},
		/**
		 * 显示左边区域
		 * @method showLeft
		...
		removeLeftContent: function(){
			justep.Component.removeComponent(this.leftCompose);
		}
	});
