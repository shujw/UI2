...
define(function(require){
	...
	var RouteState = Object.extend({
		...
		constructor: function(model){
			...
			this.ownerModel.on('onLoaded',function(){
				this.publishState(false);
				this.modelLoaded = true;
			},this);
			
			this.ownerModel.on('onActive',function(){
				if(this.modelLoaded){
					this.publishState(false);
				}
			},this);
		...
		resetState : function(hash, state){
			var self = this;
			var stateSeries = new HashbangParser(hash).parse();
			if(stateSeries.size() > 0){
				var oldStateSeries = this.stateSeries;
				oldStateSeries.each(function(key,value){
					stateSeries.add(key,value);
				},this);
				this.stateSeries = stateSeries;
				setTimeout(function(){
					self.publishState();
					self.ownerModel.postMessage({
						type:"routeState",
						newUrl:self.toHashbang()
					});
				},1000);
			}
		}
