...
define(function(require) {
	...
	var Operation = Object.extend({
		...
		_initParams : function(ctx) {
			var ret = {};
			var argsDef = !$.isFunction(this.argsDef)?this.argsDef:this.argsDef();
			if ($.isArray(argsDef)) {
				var name, p;
				for ( var i = 0; i < argsDef.length; i++) {
					name = argsDef[i].name;
					p = undefined;
					if (ctx.args && ctx.args[name]) {
						if (!ctx._argExprs)
							ctx._argExprs = {};
						if ('string' == typeof (ctx.args[name]) && ctx.args[name].indexOf('js:')===0 && !ctx._argExprs[name]) {
							try {
						...
						if (ctx._argExprs[name] instanceof Expr)
							p = Expr.eval(ctx._argExprs[name], this, ctx, ctx.$object||{});
						else
							p = ctx.args[name];
					}
					ret[name] = p;
