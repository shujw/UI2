...
define(function(require) {
	...
	Express.eval = function(exprObj, context, params, params2) {//增加表达式执行上下文中param2
		var msg;
		if (exprObj instanceof Express) {
			if (!exprObj.exprFn)
				exprObj.exprFn = Express.createExprFn(exprObj.expr);
			try {
				return exprObj.exprFn(context, params, params2);
			} catch (ex) {
	...
	Express.createExprFn = function(expr) {
		...
		var body = "with($params2||{}){with($params||{}){with($__context__||{}){return " + expr + "}}}";
		return new Function("$__context__", "$params", "$params2", body);
	};
