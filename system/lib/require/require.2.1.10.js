...
(function (global) {
    ...
    var req, s, head, baseElement, dataMain, src, interactiveScript, currentlyAddingScript, mainScript, subPath, version = '2.1.10', commentRegExp = /(\/\*([\s\S]*?)\*\/|([^:]|^)\/\/(.*)$)/mg, cjsRequireRegExp = /[^.]\s*require\s*\(\s*["']([^'"\s]+)["']\s*\)/g, jsSuffixRegExp = /\.js$/, currDirRegExp = /^\.\//, op = Object.prototype, ostring = op.toString, hasOwn = op.hasOwnProperty, ap = Array.prototype, apsp = ap.splice, isBrowser = !!(typeof window !== 'undefined'
    ...
    // to feature test w/o causing perf issues.
        readyRegExp = isBrowser && navigator.platform === 'PLAYSTATION 3' ? /^complete$/
            : /^(complete|loaded)$/, defContextName = '_',
    // Oh the tragedy, detecting opera. See the usage of isOpera for reason.
        isOpera = typeof opera !== 'undefined'
            && opera.toString() === '[object Opera]', contexts = {}, cfg = {}, globalDefQueue = [], useInteractive = false;

    // --------------hcr 为url添加版本号, 处理.a---------------------
    var deviceTypes = ["Android", "iPhone", "iPad", "iPod", "Windows Phone",
        "PC"];
    ...
    var URL = {
        ...
        isPhone: function () {
            return (URL.deviceType === "Android")
                || (URL.deviceType === "iPhone")
        ...
        enabledVLS: function () {
            ...
            if (window.__justep && window.__justep.__ResourceEngine
                && window.__justep.__ResourceEngine.url) {
        ...
        addVLS: function (url) {
            ...
            /*
             * URL._parseAUrl(info); if (info.isAUrl){ return URL._toUrl(info); }
             */
            URL._updateVLS(info);
        ...
        removeVLS: function (url) {
            if (url) {
                var items = url.split("/");
                var newItems = [];
                for (var i = 0; i < items.length; i++) {
                    if (URL._isVLS(items[i])) {
                        newItems.push(VERSION_PREFIX);
                    } else {
        ...
        _updateVLS: function (info) {
            ...
            if (appVersion) {
                var index = info.vls.indexOf(LANGUAGE_PREFIX, 1);
                if (index === -1) {
                    info.vls = VERSION_PREFIX + appVersion + LANGUAGE_PREFIX
                        + SKIN_PREFIX + DEVICE_PREFIX;
                } else {
                    info.vls = VERSION_PREFIX + appVersion
                        + info.vls.substring(index);
                }
            }
        },
        ...
        /*
         * _parseAUrl: function(info){ if (info.isAUrl){
         *
         * var i=0; var tmp = null; var wurl = null; var pathNoExt =
         * info.path.substring(0, info.path.lastIndexOf(".")); if
         * (URL.isPhone()){ for (i=0; i<(URL.Meta.newUI || []).length; i++){
         * wurl = URL._toRealUrl((URL.Meta.newUI || [])[i], pathNoExt,
         * MOBILE_EXT); if (wurl) break; }
         *
         * if (!wurl){ for (i=0; i<(URL.Meta.classicUI || []).length; i++){
         * wurl = URL._toRealUrl((URL.Meta.classicUI || [])[i], pathNoExt,
         * MOBILE_EXT); if (wurl) break; } }
         *
         * if (!wurl){ wurl = URL._toRealUrl(MOBILE_UI, pathNoExt, PC_EXT); }
         * }else if (URL.isPad()){ for (i=0; i<(URL.Meta.newUI||[]).length;
         * i++){ wurl = URL._toRealUrl((URL.Meta.newUI || [])[i], pathNoExt,
         * PAD_EXT); if (wurl) break; }
         *
         * if (!wurl){ for (i=0; i<(URL.Meta.classicUI||[]).length; i++){ wurl =
         * URL._toRealUrl((URL.Meta.classicUI || [])[i], pathNoExt, PAD_EXT); if
         * (wurl) break; } } }
         *
         * if (!wurl){ for (i=0; i<(URL.Meta.newUI||[]).length; i++){ wurl =
         * URL._toRealUrl((URL.Meta.newUI || [])[i], pathNoExt, PC_EXT); if
         * (wurl) break; }
         *
         * if (!wurl){ for (i=0; i<(URL.Meta.classicUI||[]).length; i++){ wurl =
         * URL._toRealUrl((URL.Meta.classicUI || [])[i], pathNoExt, PC_EXT); if
         * (wurl) break; } } }
         *
         * if (wurl){ info.path = wurl; info.isAUrl = false; } } }, _toRealUrl:
         * function(uiName, pathNoExt, ext){ var tmp = uiName + pathNoExt + ext;
         * if (URL.Meta.wList && (URL.Meta.wList.indexOf(tmp)!==-1)){ return
         * tmp; }else{ return null; } },
         */

        _parse: function (url) {
            var isAbsolute = (url.charAt(0) == "/");
            var result = {
                contextName: "",
                vls: "",
                path: "",
            ...
            if (!isAbsolute) {
                if (this.contextname) {
                    result.contextname = result.contextname.substring(1);
            ...
            if (!hasVls) {
                result.path = result.contextName;
                result.contextName = "";
            }
    ...
    function newContext(contextName) {
        ...
        function getUI(baseUrl) {
            ...
            /*
             * if (baseUrl){ if (baseUrl.charAt(0) == "/"){ if
             * (baseUrl.indexOf(VERSION_PREFIX) !== -1){ var items =
             * baseUrl.split("/"); //查找VERSION_PREFIX后一项 for (var i=0; i<items.length;
             * i++){ if ((items[i].indexOf(VERSION_PREFIX) == 0) && (i+1<items.length)){
             * return items[i+1]; } } }else{ var ctx =
             * window.__justep.__ResourceEngine.contextPath; if
             * (baseUrl.indexOf(ctx) === 0){ var subPath =
             * baseUrl.substring(ctx.length); return subPath.split("/")[1]; }
             *  } }else if (baseUrl.indexOf("$model") == 0){ //支持js合并之后,
             * url是$model的方式 var items = baseUrl.split("/"); if (items.length >=
             * 2){ return items[1]; } } }
             *
             * return null;
             */
        ...
        function processUI(name, baseUrl) {
            ...
            if (name && baseUrl) {
                ...
                if (result && (result.indexOf("$UI") != -1)) {
                    throw new Error("处理" + name + "中的$UI时出错, baseUrl: "
                        + baseUrl);
                }
            }
        ...
        function normalize(name, baseName, baseUrl, applyMap) {
            // slm 支持require.toUrl时候./情况下对paths的支持
            if (!baseUrl && window.__justep && window.__justep.__ResourceEngine
                && window.__justep.__ResourceEngine.contextPath) {
                baseName = config.baseUrl.replace(
                    window.__justep.__ResourceEngine.contextPath + "/",
                    processUI("$UI/", config.baseUrl));
                baseUrl = baseName + "/index.html";
            }
        ...
        function makeModuleMap(name, parentModuleMap, isNormalized, applyMap) {
            var url, pluginModule, suffix, nameParts, prefix = null, parentName = parentModuleMap ? parentModuleMap.name
                : null, parentUrl = parentModuleMap ? parentModuleMap.url
                ...
                if (prefix) {
                    if (pluginModule && pluginModule.normalize) {
                        ...
                        normalizedName = pluginModule.normalize(name, function (name) {
                            return normalize(name, parentName, parentUrl,
                                applyMap);
                        });
                    } else {
                        normalizedName = normalize(name, parentName, parentUrl,
                            applyMap);
                    }
        ...
        function checkLoaded() {
            ...
            // of 0.
                expired = waitInterval
                    && (context.startTime + waitInterval) < new Date()
                        .getTime(), noLoads = [], reqCalls = [], stillLoading = false, needCycleCheck = true;
                    ...
                    bind(
                        ...
                        function (plugin) {
                            var load, normalizedMap, normalizedMod, bundleId = getOwn(
                                ...
                                : null, parentUrl = this.map.parentMap ? this.map.parentMap.url
                                : null, localRequire = context
                                .makeRequire(map.parentMap, {
                                    enableBuildCallback: true
                                });

                            // If current map is not normalized, wait
                            // for that
                            // normalized name to load instead of
                            // continuing.
                            if (this.map.unnormalized) {
                                // Normalize the ID if the plugin allows
                                // it.
                                if (plugin.normalize) {
                                    name = plugin
                                            .normalize(
                                ...
                                // prefix and name should already be
                                // normalized, no need
                            ...
                            // If a paths config, then just load that
                            // file instead to
                            // resolve the plugin, as it is built into
                            // that paths layer.
                            if (bundleId) {
                            ...
                            load.error = bind(this, function (err) {
                                ...
                                // Remove temp unnormalized modules for
                                // this module,
                            ...
                            // Allow plugins to load other code without
                            // having to know the
                            ...
                            load.fromText = bind(
                                ...
                                function (text, textAlt) {
                                    ...
                                    // As of 2.1.0, support just
                                    // passing the text, to
                                    // reinforce
                                    // fromText only being called
                                    // once per resource. Still
                                    // support old style of passing
                                    // moduleName but discard
                                    // that moduleName in favor of
                                    // the internal ref.
                                    if (textAlt) {
                                        text = textAlt;
                                    }

                                    // Turn off interactive script
                                    // matching for IE for any
                                    // define
                                    // calls in the text, then turn
                                    // it back on at the end.
                                    if (hasInteractive) {
                                        useInteractive = false;
                                    }

                                    // Prime the system by creating
                                    // a module instance for
                                    // it.
                                    getModule(moduleMap);

                                    // Transfer any config to this
                                    // other module.
                                    ...
                                    // Bind the value of that module
                                    // to the value for this
                                    // resource ID.
                                    localRequire([moduleName],
                                        load);
                                });

                            // Use parentName here since the plugin's
                            // name is not reliable,
                            // could be some weird string with no path
        ...
        context = {
            ...
            makeRequire: function (relMap, options) {
                ...
                var css = (function () {
                    ...
                    // set to false for explicit <link> load checking when
                    // onload doesn't work perfectly (webkit)
                    var useOnload = true;

                    // trident / msie
                    if (engine[1] || engine[7])
                        useImportLoad = parseInt(engine[1]) < 6
                            || parseInt(engine[7]) <= 9;
                    // webkit
                    else if (engine[2])
                        useOnload = false;
                    ...
                    var linkLoad = function (url, callback) {
                        ...
                        if (useOnload)
                            link.onload = function () {
                                ...
                                // for style dimensions queries, a short delay
                    ...
                    return function (cssId, load) {
                        ...
                        if (!included(cssId)) {
                            ...
                            (useImportLoad ? importLoad : linkLoad)(url,
                                function () {
                                    var links = document
                ...
                mixin(
                    localRequire,
                    {
                        ...
                        /**
                         * Converts a module name + .extension into an URL
                         * path. *Requires* the use of a module name. It
                         * does not support using plain URLs like nameToUrl.
                         */
                        ...
                        toUrl: function (moduleNamePlusExt) {
                            ...
                            // Have a file extension alias, and it is not
                            // the
                            ...
                            var result = context.nameToUrl(normalize(
                                moduleNamePlusExt, relMap && relMap.id,
                                relMap && relMap.url, true), ext, true);
                            // console.log(moduleNamePlusExt + "==" +
                            // result);
                        ...
                        normalizeName: function (moduleName, varName) {
                            if (!moduleName)
                                return moduleName;
                            var url = normalize(moduleName, relMap
                                && relMap.id, relMap && relMap.url,
                                true);
                            varName = varName || "$model";
                            if (url && config && config.paths
                                && config.paths[varName]) {
                                var value = config.paths[varName];
                                if (value && (value !== "/")
                                    && (url.indexOf(value) === 0)) {
                                    url = url.substr(value.length);
                                    if (url.charAt(0) !== '/') {
                                        url += "/" + url;
                                    }
                                    url = varName + url;
                                }
                                url = URL.removeVLS(url);
                            }
                            return url;
                        },
             ...
             * for context, when this method is overriden by the optimizer. Not
            ...
            nameToUrl: function (moduleName, ext, skipExt) {
                ...
                if (req.jsExtRegExp.test(moduleName)
                    ...
                    //slm 兼容编译后require.toUrl("./xx/a.md5_xxx.png")的情况
                    moduleName = moduleName.replace(/.md5_\w+./g,'.');    
                    ...
                    for (i = syms.length; i > 0; i -= 1) {
                        ...
                        //slm 添加动态拉取path信息的逻辑
                        if(window.__justep && window.__justep.resourceMode == "md5"){
                            this.loadSubModelPaths(parentModule);
                        }
            ...
            loadSubModelPaths: function (url) {
                // console.log("loadSubModelPaths:" + url);
                var uiPath = config.paths["$UI"];

                if (/(^\$model|^\$UI|^\/)/g.test(url) == false
                    && url.indexOf(uiPath) == -1) {
                    return;
                }
                var _url = url.replace(uiPath + "/", "");
                var _urlParts = _url.split("/");
                if (_urlParts.length > 1) {
                    var appName = _urlParts[0];
                    if (_urlParts[0] == "$model" && _urlParts.length > 3) {
                        appName = _urlParts[2];
                    } else if (_urlParts[0] == "$UI" && _urlParts.length > 2) {
                        appName = _urlParts[1];
                    }
                    if (!appName || appName === "$model" || appName === "$UI") {
                        return;
                    }
                    var mappingUrl = uiPath + "/" + appName + "/" + appName
                        + "_md5_mapping_in_server.json?time="
                        + new Date().getTime();
                    // console.log("loadSubModelPaths mappingUrl:" +
                    // mappingUrl);
                    if (config.appMappingLoads
                        && config.appMappingLoads.indexOf(appName) == -1) {
                        // console.log("request mapping url:" + mappingUrl);
                        this.smartAjax(mappingUrl, function (data) {
                            mixin(config.paths, data, true, true);
                            config.appMappingLoads.push(appName);
                            // console.log("loadSubModelPaths appName:" +
                            // appName);
                        })
            ...
            // 自动发送不缓存的ajax请求 如果请求失败 发送缓存的ajax请求
            smartAjax: function (url, successCb, errorCb) {
                var xhr = (function () {
                    try {
                ...
                if ((xhr.status == 200) && xhr.responseText) {
                    ...
                    if (url.indexOf("_in_server.") != -1) {
                        var _url = url.replace("_in_server.", ".");
    ...
    each(['normalizeName', 'toUrl', 'undef', 'defined', 'specified'],
        function (prop) {
            // Reference from contexts instead of early binding to default
            // context,
            // so that during builds, the latest instance of the default
            // context
    ...
    req.load = function (context, moduleName, url) {
        ...
        if (isBrowser) {
            ...
            if (window.isInDesigner && url.substring(url.length - 3) != '.js') {// 在设计时环境下
                // zmh
                url += ".js";
            }
