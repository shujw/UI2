...
window.getMillisInHours = getMillisInHours;
...
window.getMillisInHoursMinutes=getMillisInHoursMinutes;
...
window.getMillisInDaysHoursMinutes = getMillisInDaysHoursMinutes;
...
window.millisFromHourMinute = millisFromHourMinute;
...
window.millisFromString = millisFromString;
...
window.daysFromString = daysFromString;
...
jQuery.fn.createErrorAlert = function(errorCode, message) {
  this.each(function() {
    ...
    if ($("#" + el.attr("id") + "error").size() <= 0) {
      ...
      err=$(err);
      err.attr("title",errMess).attr("src",require.toUrl("../res/alert.gif"));
      el.after(err);
    }
  });
  return this;
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  bdiv.append("<img src=\"" + require.toUrl("../res/closeBig.png") +"\" style='cursor:pointer;position:absolute;right:-40px;top:30px;'>");
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  var bg=$("<div>").attr("id","__blackpopup__");
  bg.css({position:'fixed',top:"0px",paddingTop:"50px", left:0,width:'100%',height:'100%',  backgroundImage:"url('"+require.toUrl("../res/black_70.png")+"')"});
  ...
  var img=$("<img src='" + require.toUrl("../res/closeBig.png")+"' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
  bdiv.append(img);
  img.click( function(){
    bg.trigger("close");
  });
...
function openBoardInBlack(boardId,command,params,callback){
  ...
  openBlackPopup(editUrl,$(window).width()-100,$(window).height()-50,callback );
	...
	window.createBlackPage = createBlackPage;
});