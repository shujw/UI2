define(function(require) {
	require("w!/UI2/system/components/justep/touch/demo/touchjs/mainActivity.w");
	require("w!/UI2/system/components/justep/windowDialog/demo/demo.w");
	require("w!/UI2/system/components/justep/messageDialog/demo/demo.w");
	require("w!/UI2/system/components/justep/menu/demo/base.w");
	require("w!/UI2/system/components/justep/controlGroup/demo/demo.w");
	require("w!/UI2/system/components/justep/list/demo/news.w");
	require("w!/UI2/system/components/justep/labelEdit/demo/demo.w");
	require("w!/UI2/system/components/justep/select/demo/select.w");
	require("w!/UI2/system/components/justep/button/demo/checkbox.w");
	require("w!/UI2/system/components/justep/button/demo/radio.w");
	require("w!/UI2/system/components/justep/button/demo/toggle.w");
	require("w!/UI2/system/components/justep/button/demo/button.w");
	require("w!/UI2/system/components/justep/textarea/demo/textarea.w");
	require("w!/UI2/system/components/justep/output/demo/output.w");
	require("w!/UI2/system/components/justep/input/demo/range.w");
	require("w!/UI2/system/components/justep/input/demo/input.w");
	require("w!/UI2/system/components/justep/windowContainer/demo/main.w");
	require("w!/UI2/system/components/justep/wing/demo/wing.w");
	require("w!/UI2/system/components/justep/attachment/demo/demoSimple.w");
	require("w!/UI2/system/components/justep/popMenu/demo/popMenu.w");
	require("w!/UI2/system/components/justep/popOver/demo/base.w");
	require("w!/UI2/system/components/justep/row/demo/base.w");
	require("w!/UI2/system/components/justep/scrollView/demo/list/demo.w");
	require("w!/UI2/system/components/bootstrap/row/demo/grid.w");
	require("w!/UI2/system/components/justep/panel/demo/panel.w");
	require("w!/UI2/system/components/justep/contents/demo/contents.w");
	require("w!/UI2/system/components/justep/model/demo/model.w");
	require("w!/UI2/system/components/justep/docCommon/demo/mainActivity.w");
	require("w!/UI2/demo/misc/todoMVC/index.w");
	require("w!/UI2/takeout/index.w");
	require("w!/UI2/demo/baas/complexData/index.w");
	require("w!/UI2/demo/baas/masterDetail/index.w");
	require("w!/UI2/demo/baas/simpleData/index.w");
	require("w!/UI2/demo/baas/treeData/index.w");
	require("w!/UI2/demo/baas/treeDelayLoad/index.w");
	require("w!/UI2/system/components/justep/data/demo/base.w");
	require("w!/UI2/system/components/justep/barcode/demo/demoBarcodeImage.w");
	require("w!/UI2/system/components/justep/distpicker/demo/demo.w");
	require("w!/UI2/system/components/justep/locker/demo/demo.w");
	require("w!/UI2/system/components/justep/datePicker/demo/demo.w");
	require("w!/UI2/system/components/justep/timer/demo/demo.w");
	
	require("w!/UI2/demo/register/indexActivity.w");
	require("w!/UI2/demo/tuniu/index.w");
	require("w!/UI2/demo/account/list.w");
	require("w!/UI2/demo/account/list.w");
	
	require("w!/UI2/demo/native/toast/index.w");
	require("w!/UI2/demo/native/downloader/index.w");
	require("w!/UI2/demo/native/fileOpener/index.w");
	require("w!/UI2/demo/native/open/index.w");
	require("w!/UI2/demo/native/keyboard/index.w");
	require("w!/UI2/demo/native/backgroundMode/index.w");
	require("w!/UI2/demo/native/clipboard/index.w");
	require("w!/UI2/demo/native/videoplayer/index.w");
	require("w!/UI2/demo/native/clearCache/index.w");
	require("w!/UI2/demo/native/accelerometer/mainActivity.w");
	require("w!/UI2/demo/native/camera/mainActivity.w");
	require("w!/UI2/demo/native/audio/mainActivity.w");
	require("w!/UI2/demo/native/barcode/mainActivity.w");
	require("w!/UI2/demo/native/geolocation/mainActivity.w");
	
	require("w!/UI2/demo/plugin/actionsheet/demo.w");
	require("w!/UI2/demo/plugin/appAvailability/demo.w");
	require("w!/UI2/demo/plugin/appVersion/demo.w");
	require("w!/UI2/demo/plugin/badge/demo.w");
	require("w!/UI2/demo/plugin/BLECentral/demo.w");
	require("w!/UI2/demo/plugin/baiduMap/main.w");
	require("w!/UI2/demo/plugin/calendar/demo.w");
	require("w!/UI2/demo/plugin/customURL/demo.w");
	require("w!/UI2/demo/plugin/device/demo.w");
	require("w!/UI2/demo/plugin/events/demo.w");
	require("w!/UI2/demo/plugin/iFlytek/demo.w");
	require("w!/UI2/demo/plugin/insomnia/demo.w");
	require("w!/UI2/demo/plugin/jpush/demo.w");
	require("w!/UI2/demo/plugin/notification/demo.w");
	require("w!/UI2/demo/plugin/preferences/demo.w");
	require("w!/UI2/demo/plugin/privacyscreen/demo.w");
	require("w!/UI2/demo/plugin/scanCard/demo.w");
	require("w!/UI2/demo/plugin/screenOrientation/demo.w");
	require("w!/UI2/demo/plugin/spinner/demo.w");
	require("w!/UI2/demo/plugin/statusbar/demo.w");
	require("w!/UI2/demo/plugin/template/demo.w");
	require("w!/UI2/demo/plugin/touchID/demo.w");
	require("w!/UI2/demo/plugin/voiceManager/demo.w");
	require("w!/UI2/demo/plugin/sim/demo.w");
	
	

	require("w!/UI2/demo/native/sqlite/simpleData/index.w");
	require("w!/UI2/demo/native/sqlite/treeData/index.w");
	require("w!/UI2/demo/native/sqlite/treeDelayLoad/index.w");
	

	require("w!/UI2/demo/native/share/index.w");
	require("w!/UI2/demo/native/shareInfo/index.w");
	require("w!/UI2/demo/native/voiceSearch/index.w");
	require("w!/UI2/system/api/native/demo/test.w");
	
	//require("w!/UI2/demo/takeoutNew/index.w");
	require("w!/UI2/demo/qqContacts/index.w");
	require("w!/UI2/demo/waterfall/index.w");
	require("w!/UI2/demo/dataByRestful/index.w");
	require("w!/UI2/demo/picCut/index.w");
	
	return {
		"$children" : [{
				"$children" : [{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "仿途牛",
					"psmCount" : "1",
					"url" : "/UI2/demo/tuniu/index_main.w"
				},{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "外卖",
					"psmCount" : "1",
					"url" : "/UI2/takeout/index.w"
				},/*{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "外卖(配合takeoutAdmin)",
					"psmCount" : "1",
					"url" : "/UI2/demo/takeoutNew/index.w"
				},*/{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "记账本",
					"psmCount" : "1",
					"url" : "/UI2/demo/account/list.w"
				},{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "记账本(data访问baas)",
					"psmCount" : "1",
					"url" : "/UI2/demo/account/list.w"
				},{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "QQ联系人列表",
					"psmCount" : "1",
					"url" : "/UI2/demo/qqContacts/index.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "图片裁剪",
					"psmCount" : "1",
					"url" : "/UI2/demo/picCut/mainActivity.w"
				},{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "图片瀑布流",
					"psmCount" : "1",
					"url" : "/UI2/demo/waterfall/main.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "RESTful(需单部署server)",
					"psmCount" : "1",
					"url" : "/UI2/demo/dataByRestful/index.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "简单数据",
					"psmCount" : "1",
					"url" : "/UI2/demo/baas/simpleData/index.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "复杂数据",
					"psmCount" : "1",
					"url" : "/UI2/demo/baas/complexData/index.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "树形数据（一次加载）",
					"psmCount" : "1",
					"url" : "/UI2/demo/baas/treeData/index.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "树形数据（逐级加载）",
					"psmCount" : "1",
					"url" : "/UI2/demo/baas/treeDelayLoad/index.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "主从数据",
					"psmCount" : "1",
					"url" : "/UI2/demo/baas/masterDetail/index.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "TodoMVC",
					"psmCount" : "1",
					"url" : "/UI2/demo/misc/todoMVC/index.w"
				} ,{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "内容分享1",
					"psmCount" : "1",
					"url" : "/UI2/demo/native/share/index.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "内容分享2",
					"psmCount" : "1",
					"url" : "/UI2/demo/native/shareInfo/index.w"
				},{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "JSSDK",
					"psmCount" : "1",
					"url" : "/UI2/system/api/native/demo/test.w"
				}	
					,{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "touchjs",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/touch/demo/touchjs/mainActivity.w"
				}],			
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "演示案例"
			}, {
				"$children" : [{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "model",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/model/demo/model.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "contents",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/contents/demo/contents.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "panel",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/panel/demo/panel.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "grid 9",
					"psmCount" : "1",
					"url" : "/UI2/system/components/bootstrap/row/demo/grid.w"
				/* scrollView在外卖等例子中已经体现
				}, {
					"$name" : "item",
					"$text" : "",
					"label" : "scrollView",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/scrollView/demo/list/demo.w"
				*/
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "row",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/row/demo/base.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "popOver",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/popOver/demo/base.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "popMenu",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/popMenu/demo/popMenu.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "wing",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/wing/demo/wing.w"
				},{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "attachmentSimple",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/attachment/demo/demoSimple.w"
				},{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "windowContainer",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/windowContainer/demo/main.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"label" : "input",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/input/demo/input.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "range",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/input/demo/range.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "output",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/output/demo/output.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "textarea",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/textarea/demo/textarea.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "button",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/button/demo/button.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "toggle",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/button/demo/toggle.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "radio",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/button/demo/radio.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "checkbox",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/button/demo/checkbox.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "select",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/select/demo/select.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "labelEdit",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/labelEdit/demo/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"label" : "list",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/list/demo/news.w"
				},{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "controlGroup",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/controlGroup/demo/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"label" : "menu",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/menu/demo/base.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "messageDialog",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/messageDialog/demo/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "windowDialog",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/windowDialog/demo/demo.w"
				} ,{
					"$name" : "item",
					"$text" : "",
					"label" : "data",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/data/demo/base.w"
				},{
					"$name" : "item",
					"$text" : "",
					"label" : "distpicker",
					"psmCount" : "1",
					"url" : "$UI/system/components/justep/distpicker/demo/demo.w"
				},{
					"$name" : "item",
					"$text" : "",
					"label" : "locker",
					"psmCount" : "1",
					"url" : "$UI/system/components/justep/locker/demo/demo.w"
				},{
					"$name" : "item",
					"$text" : "",
					"label" : "datePicker",
					"psmCount" : "1",
					"url" : "$UI/system/components/justep/datePicker/demo/demo.w"
				},{
					"$name" : "item",
					"$text" : "",
					"label" : "timer",
					"psmCount" : "1",
					"url" : "$UI/system/components/justep/timer/demo/demo.w"
				},
				{
					"$name" : "item",
					"$text" : "",
					"label" : "barcodeImage",
					"psmCount" : "1",
					"url" : "/UI2/system/components/justep/barcode/demo/demoBarcodeImage.w"
				}],
				"$name" : "item",
				"$text" : "",
				"label" : "组件"
			},{
			"$children" : [{
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "toast",
				"psmCount" : "1",
				"url" : "/UI2/demo/native/toast/index.w"
			}
			, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "downloader",
				"psmCount" : "1",
				"url" : "/UI2/demo/native/downloader/index.w"
			}
			, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "fileOpener",
				"psmCount" : "1",
				"url" : "/UI2/demo/native/fileOpener/index.w"
			}
			, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "open",
				"psmCount" : "1",
				"url" : "/UI2/demo/native/open/index.w"
			}
			, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "keyboard",
				"psmCount" : "1",
				"url" : "/UI2/demo/native/keyboard/index.w"
			}
			, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "backgroundMode",
				"psmCount" : "1",
				"url" : "/UI2/demo/native/backgroundMode/index.w"
			}
			, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "clipboard",
				"psmCount" : "1",
				"url" : "/UI2/demo/native/clipboard/index.w"
			}
			, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "videoplayer",
				"psmCount" : "1",
				"url" : "/UI2/demo/native/videoplayer/index.w"
			}
			, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "clearCache",
				"psmCount" : "1",
				"url" : "/UI2/demo/native/clearCache/index.w"
			},{
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "文件系统",
				"psmCount" : "1",
				"url" : "/UI2/system/components/justep/docCommon/demo/mainActivity.w"
			},{
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "摄像头拍照/录像",
					"psmCount" : "1",
					"url" : "/UI2/demo/native/camera/mainActivity.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "加速度传感器",
					"psmCount" : "1",
					"url" : "/UI2/demo/native/accelerometer/mainActivity.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "音频录制/播放",
					"psmCount" : "1",
					"url" : "/UI2/demo/native/audio/mainActivity.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "二维码扫描",
					"psmCount" : "1",
					"url" : "/UI2/demo/native/barcode/mainActivity.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "地图/地理定位",
					"psmCount" : "1",
					"url" : "/UI2/demo/native/geolocation/mainActivity.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "语音搜索",
					"psmCount" : "1",
					"url" : "/UI2/demo/native/voiceSearch/index.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "actionsheet",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/actionsheet/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "appAvailability",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/appAvailability/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "appVersion",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/appVersion/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "badge",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/badge/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "baiduMap",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/baiduMap/main.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "jPush",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/jpush/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "calendar",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/calendar/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "customURL",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/customURL/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "device",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/device/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "events",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/events/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "iFlytek",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/iFlytek/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "insomnia",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/insomnia/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "preferences",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/preferences/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "privacyscreen",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/privacyscreen/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "scanCard",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/scanCard/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "screenOrientation",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/screenOrientation/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "spinner",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/spinner/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "statusbar",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/statusbar/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "touchID",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/touchID/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "voiceManager",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/voiceManager/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "sim",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/sim/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "notification",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/notification/demo.w"
				}, {
					"$name" : "item",
					"$text" : "",
					"display" : "solid",
					"label" : "BLECentral",
					"psmCount" : "1",
					"url" : "/UI2/demo/plugin/BLECentral/demo.w"
				}
			],			
			"$name" : "item",
			"$text" : "",
			"display" : "solid",
			"label" : "cordova"
		}, {
			"$children" : [ {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "Sqlite数据存取",
				"psmCount" : "1",
				"url" : "/UI2/demo/sqlite/simpleData/index.w"
			}, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "Sqlite树形一次加载",
				"psmCount" : "1",
				"url" : "/UI2/demo/sqlite/treeData/index.w"
			}, {
				"$name" : "item",
				"$text" : "",
				"display" : "solid",
				"label" : "Sqlite树形逐级加载",
				"psmCount" : "1",
				"url" : "/UI2/demo/sqlite/treeDelayLoad/index.w"
			} ],
			"$name" : "item",
			"$text" : "",
			"display" : "solid",
			"label" : "Sqlite案例"
		} ],
		"$name" : "root",
		"$text" : ""
	};
});